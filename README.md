...
5. 執行 log 即時顯示在 GUI 下方；要中斷隨時按「停止」

### 多人 Raid（釘選模式）使用方式

遊戲內建了 Raid 釘選功能（救援列表最多可釘選 4 個 Raid）：

1. 先在遊戲裡把想打的 Raid 釘選好（最多 4 個分頁）
2. GUI 模式選「多人」，關卡選「釘選1」～「釘選4」（對應釘選分頁的順序）
3. 機器人流程：首頁 → 任務 → Raid 救援列表 → 切到指定釘選分頁 →
   檢查每場的 HP 條（可設定「HP 低於 % 時不進」）→ 加入 → 選召喚石 → 開打
4. 「Raid 選項」中可設定自動退出時間、HP 門檻等
